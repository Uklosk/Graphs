...
public class Digitizing {
     ...
     private boolean loadDataCir(){
         ...
         if(task == true){
                 ...
                 Mat circles = new Mat();
                 /*
                  * Aplicamos HoughLineP para cargra en la matriz circles todos
                  * los círculos detectados
                  */
                 Imgproc.HoughCircles( m_gray, circles, Imgproc.CV_HOUGH_GRADIENT,
                             1, 80, 70, 10, 5, 25);
                 totalcir = circles.cols();
                 /*
                  * Cargamos en la matriz de los vértices los datos de los círculos
                  * detectados:
     ...
     private boolean loadDataVec(){
         ...
         if(task == true){
                 ...
                 /* Una recta puede interpretarse como una recta fragmentada,
                  * de modo que hay que seleccionar un único fragmento con el
                  * que trabajar y eliminar los sobrantes, para ello se realiza
                  * una búsqueda que elimina las rectas cuyo punto inicial no está
                  * contenido en un vértice, y se prolongan las rectas cuyo punto
                  * final no está contenido en un vértice, la recta se prolonga
                  * hasta cortar con un vértice, por ejemplo cuando una recta
                  * corte con otra y ambas sean interpretadas como más de una recta
                  * se aplicará la prolongación hasta un vértice
                  */
                 for(int i = 0; i < t; i++){
                     ...
                     if(remain[0]==true && remain[1]==true){}
                                     ...
                                     double m = (allvectors[i][3]-allvectors[i][1])/
                                             (allvectors[i][2]-allvectors[i][0]);
                                     /* Ecuación punto-pendiente:
                                      * y - y1 = m * (x - x1)
                                      * X aumentará en el siguiente bucle y  
                                      * despejando en la anterior ecuación se  
                                      * obtiene la Y relativa, para obtener un
                                      * nuevo punto de prolongación, se comprobará
                                      * si dicho punto está en el area de un
                                      * vértice, en tal caso la prolongación
                                      * finliza, obteniendo el nuevo punto final
                                      * de la recta
                                      */
                                     double x = allvectors[i][2], y = 0;
                                     int index = 1;
                                     boolean end = false;
                                     for( ; index<m_img.width() && end != true;
                                             index++){
                                             y = m * (x+index - allvectors[i][2]) +
                                                     allvectors[i][3];
                                             end = inVertex(x+index, y);
                                     }
                                     /* Se guarda el punto obtenido como el punto
                                      * final de la recta prolongada
                                      */
                 ...
                 /* Por cada recta, HoughLinesP capta 2 líneas, una por cada borde,
                  * por tanto, de cada par de líneas detectadas sólo una será util.
                  * El siguiente código se encarga de deshechar las líneas sobrantes
                  */
                 for(int i = 0; i < t; i++)
     ...
     /*
      * En este método construimos el archivo .graph a partir de la matriz de
      * círculos y de xla matriz de rectas
      */
     public void generateXML(String timeStamp){
             File file =
                             new File(Environment.getExternalStorageDirectory()+
                             "/Graphs/",
                             "graph_"+timeStamp+".graph");
                 ...
                 serializer.setFeature(
                     ...
                     for(int i = 0; i < totalcir; i++){
                             ...
                             serializer.attribute(null, "x",
                                             Double.toString(allcir[i][0]/width));
                             serializer.attribute(null, "y",
                                             Double.toString(allcir[i][1]/height));
                             serializer.attribute(null, "r",
                                             Double.toString(allcir[i][2]));
                             String adjacent = "";
                             for(int j = 0; j < totalvec; j++)
                                     if(inVertex(allcir[i][0],allcir[i][1],
                                             allcir[i][2],allvec[j][0],
                                             allvec[j][1])
                                             ||inVertex(allcir[i][0],allcir[i][1],
                                             allcir[i][2],allvec[j][2],
                                             allvec[j][3]))
                             ...
                             if(adjacent.indexOf(",")!=-1)
                                     adjacent = adjacent.substring(0,
                                                        adjacent.lastIndexOf(","));
